...
export default function WeatherInfo({ cityName }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    // Open-MeteoのURL
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${city.latitude}&longitude=${city.longitude}&current=wind_speed_10m,relative_humidity_2m,apparent_temperature,temperature_2m,weather_code&timezone=Asia%2FTokyo`;
    // データフェッチを実行
    fetch(url)
  ...
  // 天気アイコンコンポーネント
  const iconName = weatherMap.icon;
  ...
  return (
    <div className="flex justify-around">
      <div className="flex flex-col items-center justify-center gap-8 flex-5">
        ...
        {IconComponent && <IconComponent  size={100} color='#000' />}
